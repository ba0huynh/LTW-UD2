...
// Đếm tổng số bản ghi
$count_sql_search = "SELECT COUNT(*) AS total FROM hoadon,users,quanlihoadon WHERE   hoadon.idUser=users.id and fullName LIKE '%$search%' and quanlihoadon.idBill=hoadon.idBill and (quanlihoadon.status=3 or quanlihoadon.status=2)";
$count_sql="SELECT COUNT(*) AS total FROM hoadon,quanlihoadon WHERE    quanlihoadon.idBill=hoadon.idBill and (quanlihoadon.status=2 or quanlihoadon.status=3) ";
...
$query=empty($search)?"select * from hoadon 
      ...
      where (quanlihoadon.status=3  or quanlihoadon.status=2)
      LIMIT $limit OFFSET $offset"
      :"select * from hoadon 
      join quanlihoadon on hoadon.idBill=quanlihoadon.idBill 
      join thongtinhoadon on hoadon.idBill=thongtinhoadon.idHoadon
      join users on hoadon.idUser=users.id
      where (quanlihoadon.status=3 or quanlihoadon.status=2 )and fullName like '%$search%' 
      LIMIT $limit OFFSET $offset";
...
?>
    ...
    <h2 class="text-2xl font-semibold mb-6 text-gray-800">Đơn hàng cần giao</h2>
      ...
      <table class="min-w-full border rounded-lg overflow-hidden">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <?php if ($result->num_rows == 0) { ?>
            <tr>
                <td colspan="6" class="text-center py-4">Không có đơn hàng nào cần giao</td>
            </tr>
          <?php }else{ ?>
                ...
                <td class="px-6 py-4 text-center">
                    <?php if ($row['status'] == 3) { ?>
                        <form method="POST" style="display:inline;">
                            ...
                            <button type="submit" name="giaohang" class="bg-yellow-400 text-gray-800 font-medium px-4 py-2 rounded">đang giao</button>
                        </form>
                    <?php } else if($row['status'] == 2) { ?>
                    <button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded">đã giao hàng</button>
                    <?php } ?>
                </td>
                <td class="px-6 py-4 text-center space-x-2">
                    <form method="POST" action="#" style="display:inline;">
                        <input type="hidden" name="idBill" value="1">
                        <button type="submit" name="trahang" class="text-white hover:bg-red-500  bg-red-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded">trả hàng</button>
                    </form>
