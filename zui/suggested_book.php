    ...
    while ($row2 = $result2->fetch_assoc()) { ?>
      ...
      <div class="bg-gray-50 rounded-lg sm:rounded-xl shadow-md overflow-hidden hover:shadow-xl transition duration-300 relative group">
        ...
        <div class="p-3 sm:p-4">
          <h3 class="text-base sm:text-lg font-semibold text-gray-800 line-clamp-2"><?php echo htmlspecialchars($row2["bookName"]); ?></h3>
          <div class="flex flex-wrap items-center gap-x-2 mt-2">
            <span class="text-base sm:text-lg font-bold text-red-500"><?php echo number_format($row2["currentPrice"], 0, ',', '.'); ?> đ</span>
            <span class="text-xs sm:text-sm text-gray-400 line-through"><?php echo number_format($row2["oldPrice"], 0, ',', '.'); ?> đ</span>
            <span class="text-xs sm:text-sm text-white bg-red-400 px-1.5 sm:px-2 py-0.5 rounded mt-1 sm:mt-0">-25%</span>
          </div>
          <p class="text-xs sm:text-sm text-gray-500 mt-1">Đã bán 102</p>
        </div>
      </div>
    <?php }} ?>
